...
export async function extractTransactionsFromBlob(file: Blob): Promise<ExtractResult> {
  ...
  const prompt =
    "Você é um assistente financeiro de alta precisão. Analise a imagem ou texto do documento (pode ser um recibo único ou uma fatura de cartão com dezenas de compras). Extraia TODAS as transações encontradas. Para cada transação, identifique se trata-se de uma DESPESA, de uma NOTA FISCAL/FATURAMENTO (receita), ou de uma DEVOLUÇÃO/ESTORNO (ex: 'Devolução Pix'). Classifique a transação entre 'PF' (Finanças Pessoais) ou 'PJ' (Finanças Empresariais). REGRAS DE CLASSIFICAÇÃO: 1) Despesas de IMPOSTOS (DAS, DARF, INSS, etc) são OBRIGATORIAMENTE 'PJ'. 2) Para Faturamentos/Receitas, se o pagador/origem tiver NOME DE EMPRESA (ex: Vision Med, Kirimed, LTDA, Clínica, Pagamentos S.A), ou se a descrição envolver 'PAGTO Salários' ou 'Serviços Prestados', classifique como 'PJ'. Se tiver NOME DE PESSOA FÍSICA (ex: Eric, Stephani, João, etc) e não for serviço prestado da empresa, classifique como 'PF'. IMPORTANTE: Se a transação for uma DEVOLUÇÃO, ESTORNO ou entrada de dinheiro (ex: Devolução Pix), defina o campo tipo_documento OBRIGATORIAMENTE como 'devolucao'. Identifique também o 'portador' (nome do portador do cartão, final do cartão ou 'Principal' caso não identifique adicional). Se for uma compra de mercado, conta de luz/água ou despesa conjunta, defina 'propriedade' como 'casa'. Retorne estritamente JSON contendo um array 'transacoes' preenchendo os campos descritos no schema.";
      ...
      messages: [
        {
          ...
          content: [
            ...
            { type: "file", data: base64Data, mediaType: mime },
          ],
        },
      ],
    });
